...
function Typewriter() {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (typingRef.current) {
        ...
        setColor(w.color);
        if (charRef.current >= len) { typingRef.current = false; timer = setTimeout(tick, 1400); }
        else { timer = setTimeout(tick, 85 + Math.random() * 40); }
      } else {
        charRef.current -= 1;
        setDisplay(w.label.slice(0, charRef.current));
        if (charRef.current <= 0) {
          charRef.current = 0; typingRef.current = true;
          wordRef.current = (wordRef.current + 1) % WORDS.length;
          timer = setTimeout(tick, 400);
        } else { timer = setTimeout(tick, 40 + Math.random() * 20); }
      }
    }
  ...
  return (
    <span style={{ display: "inline-flex", alignItems: "baseline", gap: "2px" }}>
      <span style={{ color, fontWeight: 700, letterSpacing: "-.01em", textShadow: `0 0 18px ${color}55`, transition: "color .4s ease" }}>{display}</span>
      <span style={{ color, opacity: blink ? 1 : 0, fontWeight: 300, fontSize: "1.05em", marginLeft: "1px", transition: "color .4s ease", textShadow: `0 0 8px ${color}` }}>|</span>
    </span>
  );
}
...
function PhotoOrb() {
  return (
    <div style={{ position: "relative", width: "clamp(200px,50vw,300px)", height: "clamp(200px,50vw,300px)", flexShrink: 0 }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{ position: "absolute", inset: `${-i * 18}px`, borderRadius: "50%", border: `1px solid rgba(0,255,231,${.12 - i * .03})`, animation: `pulse-ring ${2.5 + i}s ease-out infinite ${i * .6}s` }} />
      ))}
      <div style={{ position: "absolute", inset: "-24px", borderRadius: "50%", border: "1px dashed rgba(0,255,231,.18)", animation: "spin3d 14s linear infinite", transformStyle: "preserve-3d" }}>
      ...
      <div style={{ position: "absolute", inset: "-12px", borderRadius: "50%", border: "1px dashed rgba(167,139,250,.2)", animation: "spin3d 20s linear infinite reverse", transformStyle: "preserve-3d" }}>
      ...
      <div style={{ position: "absolute", bottom: "-4px", left: "50%", transform: "translateX(-50%)", background: "rgba(5,5,8,.92)", border: "1px solid rgba(0,255,231,.35)", borderRadius: "999px", padding: "4px 12px", display: "flex", alignItems: "center", gap: "6px", backdropFilter: "blur(12px)", whiteSpace: "nowrap", boxShadow: "0 4px 20px rgba(0,0,0,.5)", zIndex: 10 }}>
        <div style={{ width: "7px", height: "7px", borderRadius: "50%", background: "#00ffe7", boxShadow: "0 0 8px #00ffe7", animation: "pulse-dot 2s ease-in-out infinite", flexShrink: 0 }} />
        <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.55rem,.8vw,.72rem)", color: "#00ffe7", letterSpacing: ".08em" }}>AVAILABLE FOR WORK</span>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const sendEmail = async () => {
    ...
    try {
      ...
      if (err?.status === 401 || err?.status === 403) setError("Email service authentication failed.");
      else if (err?.status === 422) setError("Template error — check EmailJS template variables.");
      else setError("Failed to send. Try again or email ishworbist99@gmail.com");
    }
    setSending(false);
  };
  ...
  return (
    <>
      ...
      <div style={{ fontFamily: "'Outfit',sans-serif", background: "#050508", color: "#e2e8f0", minHeight: "100vh", width: "100%", margin: 0, padding: 0, overflowX: "hidden", position: "relative" }}>
        ...
        {/* ── NAV ── */}
        <header style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 200, height: "64px", display: "flex", alignItems: "center", padding: "0 5%", justifyContent: "space-between", background: scrolled ? "rgba(5,5,8,.95)" : "transparent", backdropFilter: scrolled ? "blur(20px)" : "none", borderBottom: scrolled ? "1px solid rgba(0,255,231,.07)" : "none", transition: "all .4s" }}>
          <div onClick={() => go("home")} style={{ cursor: "pointer", userSelect: "none", display: "flex", alignItems: "center", flexShrink: 0 }}>
            <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 900, fontSize: "clamp(1.2rem,3vw,1.6rem)", letterSpacing: "-.02em", background: "linear-gradient(135deg,#00ffe7 0%,#a78bfa 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", filter: "drop-shadow(0 0 12px rgba(0,255,231,.4))" }}>ISHWOR</span>
          ...
          <button className="mob-btn" onClick={() => setMob(m => !m)} style={{ display: "none", background: "none", border: "none", cursor: "pointer", flexDirection: "column", gap: "5px", padding: "8px", zIndex: 201 }}>
            {[0, 1, 2].map(i => <div key={i} style={{ width: "24px", height: "2px", background: "#00ffe7", borderRadius: "2px", transition: "all .3s" }} />)}
          </button>
        </header>

        {/* ── MOBILE MENU ── */}
        {mob && (
          <div style={{ position: "fixed", top: "64px", left: 0, right: 0, zIndex: 199, background: "rgba(5,5,8,.98)", backdropFilter: "blur(20px)", padding: "1.4rem 5%", display: "flex", flexDirection: "column", gap: "1rem", borderBottom: "1px solid rgba(0,255,231,.07)" }}>
            {["home", "about", "skills", "projects", "education", "experience", "contact"].map(id => (
              <span key={id} className="nav-link" onClick={() => go(id)} style={{ fontSize: "1.05rem", padding: ".4rem 0" }}>{id}</span>
            ))}
          </div>
        )}
        ...
        <section id="home" style={{ minHeight: "100vh", display: "flex", alignItems: "center", padding: "0 5%", position: "relative", zIndex: 2, background: "radial-gradient(ellipse 70% 55% at 65% 48%,rgba(114,9,183,.1) 0%,transparent 68%)" }}>
          <div style={{ maxWidth: "1100px", margin: "0 auto", width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem", alignItems: "center" }} className="hero-grid">
            <div style={{ animation: "fadeSlide 1s ease both" }}>
              {/* HELLO I'M tag */}
              <div className="hero-tag" style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.75rem,1.2vw,.92rem)", color: "#00ffe7", letterSpacing: ".14em", marginBottom: "1rem", display: "flex", alignItems: "center", gap: ".6rem" }}>
                <span style={{ fontSize: "1.4rem" }}>👋</span>
                <span>HELLO, I'M</span>
              </div>
              {/* Name */}
              <h1 style={{ fontSize: "clamp(2.8rem,8vw,5.5rem)", fontWeight: 900, lineHeight: 1.0, letterSpacing: "-.04em", marginBottom: "1rem" }}>
              ...
              {/* Typewriter */}
              <div style={{ fontSize: "clamp(1rem,2.2vw,1.4rem)", color: "#475569", marginBottom: "1.4rem", minHeight: "2rem", display: "flex", alignItems: "center", gap: ".45em", flexWrap: "wrap", justifyContent: "inherit" }}>
                <span style={{ color: "#334155", fontWeight: 500 }}>I'm a</span>
                <Typewriter />
              </div>
              {/* Bio */}
              <p className="hero-para" style={{ color: "#64748b", fontSize: "clamp(.95rem,1.5vw,1.1rem)", lineHeight: 1.8, marginBottom: "2rem", maxWidth: "400px" }}>
          ...
          {/* Scroll indicator */}
          <div style={{ position: "absolute", bottom: "1.5rem", left: "50%", transform: "translateX(-50%)", display: "flex", flexDirection: "column", alignItems: "center", gap: ".4rem", opacity: .3, zIndex: 2 }}>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: ".65rem", letterSpacing: ".14em", color: "#64748b" }}>SCROLL</div>
            <div style={{ width: "1px", height: "30px", background: "linear-gradient(to bottom,#00ffe7,transparent)" }} />
          </div>
        </section>
        ...
        <section id="about" style={{ position: "relative", zIndex: 2 }}>
          <div className="sec">
            <Reveal>
              <h2 style={{ fontSize: "clamp(1.8rem,5vw,2.8rem)", fontWeight: 900, letterSpacing: "-.03em", marginBottom: ".5rem", color: "#f1f5f9" }}>Who I Am</h2>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3.5rem", alignItems: "center" }} className="about-grid">
              <Reveal delay={.1} direction="left">
                <p style={{ color: "#64748b", fontSize: "clamp(.95rem,1.4vw,1.1rem)", lineHeight: 1.85, marginBottom: "1.2rem" }}>
                ...
                <div style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(0,255,231,.08)", borderRadius: "14px", padding: "1.2rem", marginBottom: "1.4rem" }}>
                  {[
                    ...
                    <div key={r.label} style={{ display: "flex", alignItems: "center", gap: ".8rem", marginBottom: ".55rem", flexWrap: "wrap" }}>
                      <span style={{ fontSize: "1rem", width: "20px", textAlign: "center", flexShrink: 0 }}>{r.icon}</span>
                      <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.68rem,1vw,.8rem)", color: "#475569", fontWeight: 600, letterSpacing: ".04em", minWidth: "90px" }}>{r.label}:</span>
                      <span style={{ fontSize: "clamp(.82rem,1.1vw,.95rem)", color: "#94a3b8", wordBreak: "break-word" }}>{r.val}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: ".8rem" }} className="stat-grid">
                  {[["0", "Years of Code"], ["1", "Projects Built"], ["0", "Happy Clients"], ["∞", "Cups of Tea"]].map(([n, l]) => (
                    <div key={l} style={{ background: "rgba(0,255,231,.04)", border: "1px solid rgba(0,255,231,.1)", borderRadius: "12px", padding: "1rem", textAlign: "center", animation: "borderGlow 4s ease-in-out infinite" }}>
                      <div style={{ fontSize: "clamp(1.5rem,3vw,2rem)", fontWeight: 900, color: "#00ffe7", letterSpacing: "-.04em" }}>{n}</div>
                      <div style={{ fontSize: "clamp(.68rem,1vw,.82rem)", color: "#475569", marginTop: "2px", fontWeight: 500 }}>{l}</div>
                    </div>
                  ))}
                </div>
              </Reveal>
              <Reveal delay={.2} direction="right">
                <TiltCard>
                  <div className="code-card" style={{ background: "rgba(255,255,255,.02)", border: "1px solid rgba(0,255,231,.1)", borderRadius: "20px", padding: "1.8rem", fontFamily: "'DM Mono',monospace", fontSize: "clamp(.78rem,1.1vw,.92rem)", lineHeight: 1.9, color: "#475569", boxShadow: "0 40px 80px rgba(0,0,0,.4)", overflowX: "auto" }}>
                    ...
                    <div>{"}"}</div>
                    <div style={{ marginTop: ".6rem", color: "#2d3748" }}>{"// always learning, always shipping"}</div>
                  </div>
                </TiltCard>
              </Reveal>
            </div>
        ...
        <section id="projects" style={{ position: "relative", zIndex: 2 }}>
          <div className="sec">
            <Reveal>
              <h2 style={{ fontSize: "clamp(1.8rem,5vw,2.8rem)", fontWeight: 900, letterSpacing: "-.03em", marginBottom: ".5rem", color: "#f1f5f9" }}>Featured Work</h2>
              <div style={{ width: "36px", height: "2px", background: "#00ffe7", marginBottom: "2.5rem" }} />
            </Reveal>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(260px,1fr))", gap: "1.2rem" }} className="proj-grid">
              {PROJECTS.map((p, i) => (
                <Reveal key={p.title} delay={i * .08}>
                  <TiltCard style={{ height: "100%" }}>
                    <div className="proj-card">
                      ...
                      <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.65rem,.9vw,.78rem)", color: p.color, letterSpacing: ".1em", marginBottom: ".7rem", opacity: .7 }}>FEATURED PROJECT</div>
                      <h3 style={{ fontSize: "clamp(1.1rem,1.8vw,1.3rem)", fontWeight: 800, color: "#f1f5f9", marginBottom: ".6rem", letterSpacing: "-.02em" }}>{p.title}</h3>
                      <p style={{ color: "#64748b", fontSize: "clamp(.85rem,1.1vw,.95rem)", lineHeight: 1.7, marginBottom: "1.1rem" }}>{p.desc}</p>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: ".4rem" }}>
                        {p.tags.map(t => <span key={t} style={{ background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.08)", borderRadius: "6px", padding: "2px 8px", fontSize: "clamp(.65rem,.85vw,.78rem)", fontFamily: "'DM Mono',monospace", color: "#64748b" }}>{t}</span>)}
                      </div>
                    </div>
                  </TiltCard>
                </Reveal>
        ...
        <section id="education" style={{ position: "relative", zIndex: 2, background: "rgba(255,255,255,.012)" }}>
          <div className="sec">
            <Reveal>
              <h2 style={{ fontSize: "clamp(1.8rem,5vw,2.8rem)", fontWeight: 900, letterSpacing: "-.03em", marginBottom: ".5rem", color: "#f1f5f9" }}>Academic Background</h2>
              <div style={{ width: "36px", height: "2px", background: "#a78bfa", marginBottom: "2.5rem" }} />
            </Reveal>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(320px,1fr))", gap: "1.2rem" }} className="edu-grid">
              {EDUCATION.map((e, i) => (
                  ...
                  <TiltCard style={{ height: "100%" }}>
                    <div className="edu-card" style={{ height: "100%" }}>
                      ...
                      <div style={{ display: "flex", alignItems: "flex-start", gap: ".9rem" }}>
                        <div style={{ width: "46px", height: "46px", borderRadius: "12px", background: "rgba(167,139,250,.1)", border: "1px solid rgba(167,139,250,.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.3rem", flexShrink: 0 }}>{e.icon}</div>
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: ".4rem", marginBottom: ".4rem" }}>
                            <h3 style={{ fontWeight: 800, fontSize: "clamp(.92rem,1.3vw,1.05rem)", color: "#f1f5f9", flex: 1, minWidth: "140px" }}>{e.degree}</h3>
                            <div style={{ display: "flex", alignItems: "center", gap: ".35rem", flexWrap: "wrap" }}>
                              {e.running && (
                                <span style={{ display: "flex", alignItems: "center", gap: "4px", fontFamily: "'DM Mono',monospace", fontSize: "clamp(.62rem,.85vw,.74rem)", color: "#34d399", background: "rgba(52,211,153,.1)", border: "1px solid rgba(52,211,153,.25)", borderRadius: "999px", padding: "2px 8px", whiteSpace: "nowrap" }}>
                                  <span style={{ width: "5px", height: "5px", borderRadius: "50%", background: "#34d399", boxShadow: "0 0 6px #34d399", display: "inline-block", animation: "pulse-dot 2s ease-in-out infinite", flexShrink: 0 }} />
                                  Studying
                                </span>
                              )}
                              <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.62rem,.85vw,.74rem)", color: "#a78bfa", background: "rgba(167,139,250,.1)", border: "1px solid rgba(167,139,250,.2)", borderRadius: "6px", padding: "2px 8px", whiteSpace: "nowrap" }}>{e.period}</span>
                            </div>
                          </div>
                          <div style={{ fontSize: "clamp(.78rem,1vw,.88rem)", color: "#a78bfa", marginBottom: ".55rem", fontWeight: 600 }}>{e.school}</div>
                          <p style={{ color: "#64748b", fontSize: "clamp(.82rem,1.1vw,.92rem)", lineHeight: 1.7 }}>{e.desc}</p>
                        </div>
                      </div>
                    </div>
                  </TiltCard>
        ...
        <section id="experience" style={{ position: "relative", zIndex: 2 }}>
          <div className="sec">
            <Reveal>
              <h2 style={{ fontSize: "clamp(1.8rem,5vw,2.8rem)", fontWeight: 900, letterSpacing: "-.03em", marginBottom: ".5rem", color: "#f1f5f9" }}>Experience</h2>
              <div style={{ width: "36px", height: "2px", background: "#00ffe7", marginBottom: "2.5rem" }} />
            </Reveal>
            <Reveal delay={.1}>
              <TiltCard>
                <div style={{ background: "rgba(0,255,231,.04)", border: "1px solid rgba(0,255,231,.2)", borderRadius: "20px", padding: "clamp(1.4rem,3vw,2.4rem)", position: "relative", overflow: "hidden" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", gap: "1.2rem", flexWrap: "wrap" }}>
                    <div style={{ width: "54px", height: "54px", borderRadius: "14px", background: "rgba(0,255,231,.08)", border: "1px solid rgba(0,255,231,.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.6rem", flexShrink: 0, animation: "borderGlow 3s ease-in-out infinite" }}>🚀</div>
                    <div style={{ flex: 1, minWidth: "200px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: ".5rem", marginBottom: ".5rem" }}>
                        <h3 style={{ fontWeight: 900, fontSize: "clamp(1.1rem,2.5vw,1.4rem)", color: "#00ffe7" }}>Just Getting Started!</h3>
                        <span style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.72rem,1vw,.84rem)", color: "#00ffe7", background: "rgba(0,255,231,.08)", border: "1px solid rgba(0,255,231,.2)", borderRadius: "6px", padding: "3px 10px", whiteSpace: "nowrap" }}>2024 – Present</span>
                      </div>
                      <div style={{ fontSize: "clamp(.82rem,1.1vw,.94rem)", color: "#475569", marginBottom: ".8rem", fontWeight: 600 }}>Open to Opportunities · Kathmandu, Nepal</div>
                      <p style={{ color: "#94a3b8", fontSize: "clamp(.88rem,1.2vw,1rem)", lineHeight: 1.85, marginBottom: "1.2rem" }}>
                      ...
                      <div className="exp-tags" style={{ display: "flex", flexWrap: "wrap", gap: ".5rem" }}>
                        {["🟢 Available Now", "💻 Building Projects", "📚 Always Learning", "🤝 Open to Internships"].map(tag => (
                          <span key={tag} style={{ background: "rgba(0,255,231,.06)", border: "1px solid rgba(0,255,231,.15)", borderRadius: "999px", padding: "5px 12px", fontSize: "clamp(.78rem,1vw,.88rem)", color: "#00ffe7", fontWeight: 600 }}>{tag}</span>
                        ))}
                      </div>
                    </div>
                  </div>
        ...
        <section id="contact" style={{ position: "relative", zIndex: 2, background: "rgba(255,255,255,.012)" }}>
          <div className="sec">
            <Reveal>
              <h2 style={{ fontSize: "clamp(1.8rem,5vw,2.8rem)", fontWeight: 900, letterSpacing: "-.03em", marginBottom: ".5rem", color: "#f1f5f9" }}>Let's Build Together</h2>
              <div style={{ width: "36px", height: "2px", background: "#00ffe7", marginBottom: "2.5rem" }} />
            </Reveal>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3.5rem", alignItems: "start" }} className="contact-grid">
              <Reveal delay={.1} direction="left">
                <p style={{ color: "#64748b", fontSize: "clamp(.92rem,1.3vw,1.05rem)", lineHeight: 1.8, marginBottom: "1.8rem" }}>
                ...
                {[
                  ...
                  { icon: "📞", label: "Phone", val: "+977 9811630107" },
                  { icon: "🐙", label: "GitHub", val: "github.com/Bist0" },
                ].map(c => (
                  <div key={c.label} style={{ display: "flex", alignItems: "center", gap: ".8rem", marginBottom: ".8rem" }}>
                    <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: "rgba(0,255,231,.06)", border: "1px solid rgba(0,255,231,.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1rem", flexShrink: 0 }}>{c.icon}</div>
                    <div style={{ minWidth: 0 }}>
                      <div style={{ fontSize: "clamp(.62rem,.85vw,.72rem)", color: "#334155", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase", marginBottom: "1px" }}>{c.label}</div>
                      <div style={{ fontSize: "clamp(.8rem,1.1vw,.9rem)", color: "#94a3b8", wordBreak: "break-word" }}>{c.val}</div>
                    </div>
                  </div>
                ))}
              </Reveal>
              <Reveal delay={.2} direction="right">
                {sent ? (
                  <div style={{ textAlign: "center", padding: "3rem 1.5rem" }}>
                    ...
                    <div style={{ fontSize: "1.2rem", fontWeight: 700, color: "#00ffe7", marginBottom: ".4rem" }}>Message Sent!</div>
                    <div style={{ color: "#64748b", fontSize: ".95rem", marginBottom: "1.5rem" }}>I'll get back to you within 24 hours.</div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: ".8rem" }}>
                    <input className="c-input" placeholder="Your Name *" value={formData.name} onChange={e => setFormData(p => ({ ...p, name: e.target.value }))} />
                    <input className="c-input" placeholder="Your Email *" type="email" value={formData.email} onChange={e => setFormData(p => ({ ...p, email: e.target.value }))} />
                    <input className="c-input" placeholder="Subject" value={formData.subject} onChange={e => setFormData(p => ({ ...p, subject: e.target.value }))} />
                    <textarea className="c-input" placeholder="Tell me about your project... *" rows={5} style={{ resize: "vertical" }} value={formData.message} onChange={e => setFormData(p => ({ ...p, message: e.target.value }))} />
                    {error && (
                      <div style={{ color: "#ff2d78", fontSize: ".9rem", marginTop: "-.2rem", display: "flex", alignItems: "center", gap: ".4rem" }}>
                        <span>⚠️</span> {error}
                      </div>
                    )}
                    <button className="btn-glow" onClick={sendEmail} disabled={sending} style={{ width: "100%", marginTop: ".2rem" }}>
        ...
        <footer style={{ position: "relative", zIndex: 2, borderTop: "1px solid rgba(255,255,255,.07)", background: "#050508" }}>
          <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "3rem 5% 2rem", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: "2.5rem" }} className="footer-grid">
            <div>
              <div style={{ userSelect: "none", marginBottom: ".9rem", display: "flex", alignItems: "center" }}>
                <span style={{ fontFamily: "'Outfit',sans-serif", fontWeight: 900, fontSize: "clamp(1.2rem,2.5vw,1.6rem)", letterSpacing: "-.02em", background: "linear-gradient(135deg,#00ffe7 0%,#a78bfa 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text", filter: "drop-shadow(0 0 10px rgba(0,255,231,.4))" }}>ISHWOR</span>
              ...
              <p style={{ color: "#64748b", fontSize: "clamp(.82rem,1.1vw,.92rem)", lineHeight: 1.8, marginBottom: "1.4rem", maxWidth: "260px" }}>
                Hi, I'm Ishwor Bist — a web developer from Nepal passionate about building responsive, user-friendly web applications.
              </p>
              <div style={{ display: "flex", gap: ".6rem" }}>
                {[
                  ...
                  <a key={s.label} href={s.href} target="_blank" rel="noreferrer" title={s.label}
                    style={{ width: "40px", height: "40px", borderRadius: "50%", background: "rgba(255,255,255,.05)", border: "1px solid rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none", transition: "all .25s", cursor: "pointer", color: "#94a3b8" }}
            ...
            <div>
              <h4 style={{ fontSize: "clamp(.9rem,1.3vw,1rem)", fontWeight: 700, color: "#f1f5f9", marginBottom: "1.2rem", letterSpacing: ".04em" }}>Quick Links</h4>
              <div style={{ display: "flex", flexDirection: "column", gap: ".6rem" }}>
                {["home", "about", "skills", "projects", "education", "experience", "contact"].map(id => (
                  <span key={id} onClick={() => go(id)} style={{ display: "flex", alignItems: "center", gap: ".5rem", color: "#64748b", fontSize: "clamp(.82rem,1.1vw,.92rem)", cursor: "pointer", transition: "color .2s" }}
                    onMouseEnter={e => e.currentTarget.style.color = "#00ffe7"}
                    onMouseLeave={e => e.currentTarget.style.color = "#64748b"}
                  >
                    <span style={{ color: "#00ffe7", fontSize: ".8rem" }}>→</span>
            ...
            <div>
              <h4 style={{ fontSize: "clamp(.9rem,1.3vw,1rem)", fontWeight: 700, color: "#f1f5f9", marginBottom: "1.2rem", letterSpacing: ".04em" }}>Contact Information</h4>
              <div style={{ display: "flex", flexDirection: "column", gap: ".9rem" }}>
                {[
                  ...
                  <div key={c.val} style={{ display: "flex", alignItems: "flex-start", gap: ".7rem" }}>
                    <div style={{ width: "32px", height: "32px", borderRadius: "8px", background: "rgba(0,255,231,.06)", border: "1px solid rgba(0,255,231,.12)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: ".85rem", flexShrink: 0 }}>{c.icon}</div>
                    <span style={{ color: "#64748b", fontSize: "clamp(.78rem,1vw,.88rem)", lineHeight: 1.5, paddingTop: "5px", wordBreak: "break-word" }}>{c.val}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ borderTop: "1px solid rgba(255,255,255,.05)", padding: "1.2rem 5%", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: ".6rem", maxWidth: "1100px", margin: "0 auto" }}>
            <div style={{ fontFamily: "'DM Mono',monospace", fontSize: "clamp(.68rem,.9vw,.78rem)", color: "#8aa4be" }}>
